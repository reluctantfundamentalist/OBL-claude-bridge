#!/usr/bin/env python3
"""Feishu init hook — loads brain context on session start/resume for Feishu sessions.

Identical to init.py but with identity hints tuned for the Feishu assistant identity.
"""
...
# Different identity hints for Feishu bot
...
def emit_cron_reminder():
    ...
    print("On your first response, run CronList. For any of these jobs not present, call CronCreate using the cron + prompt fields from the spec file (skip entries marked one_shot=true):")
    for j in jobs:
        if j.get("one_shot"):
            continue
...
def main():
    ...
    if identity:
        print(f"## Brain Context (Feishu session init)\n{identity}")
        print("Brain context loaded on Feishu session start.", file=sys.stderr)
    else:
